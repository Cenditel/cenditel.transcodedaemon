...
class ITranscodeSetings(Interface):
    ...
    transcode_switch = schema.Bool(title=_(u"Transcode Switch"),
                                   description=_(u"Specifies if the transcode is ON or OFF, \n"
                                               u"default is True"),
                                   default=True)

    adress_of_streaming_server = schema.Text(title=_(u"The adress of the streaming server, could be apache or nginx"),
                                  description=_(u"The adress of the streaming server",
                                                default=_(u"Enter the URL to the streaming server")
                                                        ),
                                  required=True,
    ...
    mount_point_fss = schema.Text(title=_(u"The mount point of File System Storage"),
                                  description=_(u"Enter the path of the mount point",
                                                default=_(u"Enter the path of the mount point")
                                                        ),
                                  required=True,
    ...
    max_file_size = schema.Int(title=_(u"The max file size that can be uploaded"),
                                       description=_(u"Enter the max file size, in MB (like 30)",
                                                default=_(u"Enter the max file size, in MB (like 30)")
                                                    ),
                                  required=True,
                                  default = 30,
                                  min = 5,
    ...
    if audio==1:
        ...
        video_valid_content_types = schema.Text(title=_(u"Video Valid Content Types to be uploaded"),
                                  description=_(u"Those are the valid content types that can be uploaded",
                                                default= _(description_valid_content_types)),
                                  required=True,
                                  default=_(u"video/3gpp video/mpeg video/quicktime video/x-flv video/x-mng video/x-ms-wmv video/x-msvideo video/ogg video/mp4 video/x-ms-wmv "))
    else:
        pass
    if video==1:
        ...
        audio_valid_content_types = schema.Text(title=_(u"Video Valid Content Types to be uploaded"),
                                  description=_(u"Those are the valid content types that can be uploaded",
                                                default= _(description_valid_content_types)),
                                  required=True,
                                  default=_(u"audio/midi audio/mpeg audio/x-realaudio "))
    else:
        pass
